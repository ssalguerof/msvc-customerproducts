...
import static org.bank.ssalguerof.msvc.customerproducts.utils.Constantes.COD_TRANS_CARGACONSUMO;
import static org.bank.ssalguerof.msvc.customerproducts.utils.Constantes.COD_TRANS_DEPOCTA;
import static org.bank.ssalguerof.msvc.customerproducts.utils.Constantes.COD_TRANS_PAGOCREDITO;
import static org.bank.ssalguerof.msvc.customerproducts.utils.Constantes.COD_TRANS_RETICTA;
            ...
            .flatMap(hasElement -> {
              if (hasElement) {
                ...
                return Mono.error(new RuntimeException("El cliente ya cuenta con el Producto con "
                  + "el código de producto CTAAHO, CTACOR o CTAPLZ"));
              } else {
  ...
  public Mono<CustomerProduct> updateProductTransaction(
    ...
    return customerProductDao.findById(idCustomerProd)
      .flatMap(customerProduct -> {
        ...
        if (transaction.getCodTipoMovimiento().equals(COD_TRANS_DEPOCTA)
        ...
        if (transaction.getCodTipoMovimiento().equals(COD_TRANS_DEPOCTA) && customerProduct
            .getCodProducto().equals(COD_CTACORRIENTE)) {
        ...
        if (transaction.getCodTipoMovimiento().equals(COD_TRANS_RETICTA)
            && customerProduct.getCodProducto().equals(COD_CTAAHORRO)) {
        ...
        if (transaction.getCodTipoMovimiento().equals(COD_TRANS_PAGOCREDITO)
        ...
        if (transaction.getCodTipoMovimiento().equals(COD_TRANS_CARGACONSUMO)
            && customerProduct.getCodProducto().equals(COD_TARJCREDITO)) {
